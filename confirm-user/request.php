...
if (!empty($_POST)) {
    ...
    $user_of_email = get_by_options('users', $get_user_email_option);
    ...
    if (mysqli_num_rows(mysqli_query($conn, "SELECT user_username FROM users WHERE user_username='$username'")) > 0) {
    ...
    } elseif (mysqli_num_rows(mysqli_query($conn, "SELECT user_email FROM users WHERE user_email='$email' and verified = 0")) > 0) {
        ...
        try {
            $verificationCode = md5(uniqid("Email của bạn chưa active. Nhấn vào đây để active nhé!", true)); //https://www.php.net/manual/en/function.uniqid
            ...
            $mail->Subject = 'Verification Users | PHP Training | Subscription | By Tân Hồng IT';
            $mail->Body = $htmlStr;
...
// require('confirm-user/result.php');
